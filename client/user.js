        ...
        if (note && nbAvis) {
            ...
            if (id) {
                Meteor.call("getBlablaInfoswithID", id, function (error, result) {
                    var infos_set = {
                        "note": result.data.results.collection1[0].note,
                        "nbAvis": result.data.results.collection1[0].nb_avis
                    };
                ...
                Meteor.users.update({_id: this._id}, {
                    $set: {
                        "profile.blablaNote": infos.note,
                        "profile.blablaNbAvis": infos.nbAvis,
                        "profile.linkedAcc": linked
                    }
                });
                return infos;
            }
    ...
    ebayInfos: function () {
        ...
        if (note && noteTrust) {
            ...
            if (username) {
                ...
                Meteor.users.update({_id: this._id}, {
                    $set: {
                        "profile.ebayNote": infos.note,
                        "profile.ebayNoteTrust": infos.noteTrust,
                        "profile.linkedAcc": linked
                    }
                });
                return infos;
    ...
    bnbInfos: function () {
        ...
        if (note && nbAvis) {
            ...
            if (id) {
                ...
                Meteor.users.update({_id: this._id}, {
                    $set: {
                        "profile.bnbNote": infos.note,
                        "profile.bnbNbAvis": infos.nbAvis,
                        "profile.linkedAcc": linked
                    }
                });
                return infos;
    ...
    'click #blabla-delete': function (event, template) {
        ...
        Meteor.users.update({_id: this._id}, {
            $unset: {
                "profile.blablaId": "",
                "profile.blablaNote": "",
                "profile.blablaNbAvis": ""
            }, $set: {"profile.linkedAcc": linked}
        });
    },
